          ...
          <>
            {pinned.length > 0 && (
              <>
                ...
                <NoteGrid notes={pinned} onOpen={setActive} onTogglePin={togglePin} />
                {others.length > 0 && <p className="text-xs font-semibold uppercase tracking-wider text-[#9CA3AF] mt-8 mb-3">Others</p>}
              </>
            )}
            <NoteGrid notes={others} onOpen={setActive} onTogglePin={togglePin} />
          </>
        )}
      </div>
...
function NoteGrid({ notes, onOpen, onTogglePin }: {
  notes: Note[];
  onOpen: (n: Note) => void;
        ...
        >
          <div className="flex items-start justify-between gap-2 mb-1.5">
            ...
            <span
              ...
              className={`shrink-0 p-1 rounded-lg transition-colors ${note.is_pinned ? 'text-[#D97706]' : 'text-[#C4C4BC] opacity-0 group-hover:opacity-100 hover:text-[#1A1A1A]'}`}
            >
